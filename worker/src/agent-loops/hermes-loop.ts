/**
 * Hermes Agent Loop — Uses OpenRouter with tool calling.
 * NousResearch Hermes format: standard OpenAI-compatible tool calling
 * via OpenRouter. The model receives tools, makes tool_calls, and
 * we execute them with simulated handlers.
 */
...
export class HermesLoop implements AgentLoop {
        ...
        if (choice.message.tool_calls && choice.message.tool_calls.length > 0) {
          ...
          for (const toolCall of choice.message.tool_calls) {
            ...
            turnMetrics.push({
              ...
              latencyMs,
              costUsd: 0, // Calculated from OpenRouter pricing
